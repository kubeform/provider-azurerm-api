/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/hpc/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeCacheBlobTargets implements CacheBlobTargetInterface
type FakeCacheBlobTargets struct {
	Fake *FakeHpcV1alpha1
	ns   string
}

var cacheblobtargetsResource = schema.GroupVersionResource{Group: "hpc.azurerm.kubeform.com", Version: "v1alpha1", Resource: "cacheblobtargets"}

var cacheblobtargetsKind = schema.GroupVersionKind{Group: "hpc.azurerm.kubeform.com", Version: "v1alpha1", Kind: "CacheBlobTarget"}

// Get takes name of the cacheBlobTarget, and returns the corresponding cacheBlobTarget object, and an error if there is any.
func (c *FakeCacheBlobTargets) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.CacheBlobTarget, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(cacheblobtargetsResource, c.ns, name), &v1alpha1.CacheBlobTarget{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.CacheBlobTarget), err
}

// List takes label and field selectors, and returns the list of CacheBlobTargets that match those selectors.
func (c *FakeCacheBlobTargets) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.CacheBlobTargetList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(cacheblobtargetsResource, cacheblobtargetsKind, c.ns, opts), &v1alpha1.CacheBlobTargetList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.CacheBlobTargetList{ListMeta: obj.(*v1alpha1.CacheBlobTargetList).ListMeta}
	for _, item := range obj.(*v1alpha1.CacheBlobTargetList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested cacheBlobTargets.
func (c *FakeCacheBlobTargets) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(cacheblobtargetsResource, c.ns, opts))

}

// Create takes the representation of a cacheBlobTarget and creates it.  Returns the server's representation of the cacheBlobTarget, and an error, if there is any.
func (c *FakeCacheBlobTargets) Create(ctx context.Context, cacheBlobTarget *v1alpha1.CacheBlobTarget, opts v1.CreateOptions) (result *v1alpha1.CacheBlobTarget, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(cacheblobtargetsResource, c.ns, cacheBlobTarget), &v1alpha1.CacheBlobTarget{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.CacheBlobTarget), err
}

// Update takes the representation of a cacheBlobTarget and updates it. Returns the server's representation of the cacheBlobTarget, and an error, if there is any.
func (c *FakeCacheBlobTargets) Update(ctx context.Context, cacheBlobTarget *v1alpha1.CacheBlobTarget, opts v1.UpdateOptions) (result *v1alpha1.CacheBlobTarget, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(cacheblobtargetsResource, c.ns, cacheBlobTarget), &v1alpha1.CacheBlobTarget{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.CacheBlobTarget), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeCacheBlobTargets) UpdateStatus(ctx context.Context, cacheBlobTarget *v1alpha1.CacheBlobTarget, opts v1.UpdateOptions) (*v1alpha1.CacheBlobTarget, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(cacheblobtargetsResource, "status", c.ns, cacheBlobTarget), &v1alpha1.CacheBlobTarget{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.CacheBlobTarget), err
}

// Delete takes name of the cacheBlobTarget and deletes it. Returns an error if one occurs.
func (c *FakeCacheBlobTargets) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(cacheblobtargetsResource, c.ns, name), &v1alpha1.CacheBlobTarget{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeCacheBlobTargets) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(cacheblobtargetsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.CacheBlobTargetList{})
	return err
}

// Patch applies the patch and returns the patched cacheBlobTarget.
func (c *FakeCacheBlobTargets) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.CacheBlobTarget, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(cacheblobtargetsResource, c.ns, name, pt, data, subresources...), &v1alpha1.CacheBlobTarget{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.CacheBlobTarget), err
}
