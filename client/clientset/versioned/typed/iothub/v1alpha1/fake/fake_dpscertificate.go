/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/iothub/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDpsCertificates implements DpsCertificateInterface
type FakeDpsCertificates struct {
	Fake *FakeIothubV1alpha1
	ns   string
}

var dpscertificatesResource = schema.GroupVersionResource{Group: "iothub.azurerm.kubeform.com", Version: "v1alpha1", Resource: "dpscertificates"}

var dpscertificatesKind = schema.GroupVersionKind{Group: "iothub.azurerm.kubeform.com", Version: "v1alpha1", Kind: "DpsCertificate"}

// Get takes name of the dpsCertificate, and returns the corresponding dpsCertificate object, and an error if there is any.
func (c *FakeDpsCertificates) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DpsCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(dpscertificatesResource, c.ns, name), &v1alpha1.DpsCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DpsCertificate), err
}

// List takes label and field selectors, and returns the list of DpsCertificates that match those selectors.
func (c *FakeDpsCertificates) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DpsCertificateList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(dpscertificatesResource, dpscertificatesKind, c.ns, opts), &v1alpha1.DpsCertificateList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DpsCertificateList{ListMeta: obj.(*v1alpha1.DpsCertificateList).ListMeta}
	for _, item := range obj.(*v1alpha1.DpsCertificateList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested dpsCertificates.
func (c *FakeDpsCertificates) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(dpscertificatesResource, c.ns, opts))

}

// Create takes the representation of a dpsCertificate and creates it.  Returns the server's representation of the dpsCertificate, and an error, if there is any.
func (c *FakeDpsCertificates) Create(ctx context.Context, dpsCertificate *v1alpha1.DpsCertificate, opts v1.CreateOptions) (result *v1alpha1.DpsCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(dpscertificatesResource, c.ns, dpsCertificate), &v1alpha1.DpsCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DpsCertificate), err
}

// Update takes the representation of a dpsCertificate and updates it. Returns the server's representation of the dpsCertificate, and an error, if there is any.
func (c *FakeDpsCertificates) Update(ctx context.Context, dpsCertificate *v1alpha1.DpsCertificate, opts v1.UpdateOptions) (result *v1alpha1.DpsCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(dpscertificatesResource, c.ns, dpsCertificate), &v1alpha1.DpsCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DpsCertificate), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDpsCertificates) UpdateStatus(ctx context.Context, dpsCertificate *v1alpha1.DpsCertificate, opts v1.UpdateOptions) (*v1alpha1.DpsCertificate, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(dpscertificatesResource, "status", c.ns, dpsCertificate), &v1alpha1.DpsCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DpsCertificate), err
}

// Delete takes name of the dpsCertificate and deletes it. Returns an error if one occurs.
func (c *FakeDpsCertificates) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(dpscertificatesResource, c.ns, name), &v1alpha1.DpsCertificate{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDpsCertificates) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(dpscertificatesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DpsCertificateList{})
	return err
}

// Patch applies the patch and returns the patched dpsCertificate.
func (c *FakeDpsCertificates) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DpsCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(dpscertificatesResource, c.ns, name, pt, data, subresources...), &v1alpha1.DpsCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DpsCertificate), err
}
