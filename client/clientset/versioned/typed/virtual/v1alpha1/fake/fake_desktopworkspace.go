/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/virtual/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDesktopWorkspaces implements DesktopWorkspaceInterface
type FakeDesktopWorkspaces struct {
	Fake *FakeVirtualV1alpha1
	ns   string
}

var desktopworkspacesResource = schema.GroupVersionResource{Group: "virtual.azurerm.kubeform.com", Version: "v1alpha1", Resource: "desktopworkspaces"}

var desktopworkspacesKind = schema.GroupVersionKind{Group: "virtual.azurerm.kubeform.com", Version: "v1alpha1", Kind: "DesktopWorkspace"}

// Get takes name of the desktopWorkspace, and returns the corresponding desktopWorkspace object, and an error if there is any.
func (c *FakeDesktopWorkspaces) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DesktopWorkspace, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(desktopworkspacesResource, c.ns, name), &v1alpha1.DesktopWorkspace{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopWorkspace), err
}

// List takes label and field selectors, and returns the list of DesktopWorkspaces that match those selectors.
func (c *FakeDesktopWorkspaces) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DesktopWorkspaceList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(desktopworkspacesResource, desktopworkspacesKind, c.ns, opts), &v1alpha1.DesktopWorkspaceList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DesktopWorkspaceList{ListMeta: obj.(*v1alpha1.DesktopWorkspaceList).ListMeta}
	for _, item := range obj.(*v1alpha1.DesktopWorkspaceList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested desktopWorkspaces.
func (c *FakeDesktopWorkspaces) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(desktopworkspacesResource, c.ns, opts))

}

// Create takes the representation of a desktopWorkspace and creates it.  Returns the server's representation of the desktopWorkspace, and an error, if there is any.
func (c *FakeDesktopWorkspaces) Create(ctx context.Context, desktopWorkspace *v1alpha1.DesktopWorkspace, opts v1.CreateOptions) (result *v1alpha1.DesktopWorkspace, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(desktopworkspacesResource, c.ns, desktopWorkspace), &v1alpha1.DesktopWorkspace{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopWorkspace), err
}

// Update takes the representation of a desktopWorkspace and updates it. Returns the server's representation of the desktopWorkspace, and an error, if there is any.
func (c *FakeDesktopWorkspaces) Update(ctx context.Context, desktopWorkspace *v1alpha1.DesktopWorkspace, opts v1.UpdateOptions) (result *v1alpha1.DesktopWorkspace, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(desktopworkspacesResource, c.ns, desktopWorkspace), &v1alpha1.DesktopWorkspace{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopWorkspace), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDesktopWorkspaces) UpdateStatus(ctx context.Context, desktopWorkspace *v1alpha1.DesktopWorkspace, opts v1.UpdateOptions) (*v1alpha1.DesktopWorkspace, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(desktopworkspacesResource, "status", c.ns, desktopWorkspace), &v1alpha1.DesktopWorkspace{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopWorkspace), err
}

// Delete takes name of the desktopWorkspace and deletes it. Returns an error if one occurs.
func (c *FakeDesktopWorkspaces) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(desktopworkspacesResource, c.ns, name), &v1alpha1.DesktopWorkspace{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDesktopWorkspaces) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(desktopworkspacesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DesktopWorkspaceList{})
	return err
}

// Patch applies the patch and returns the patched desktopWorkspace.
func (c *FakeDesktopWorkspaces) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DesktopWorkspace, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(desktopworkspacesResource, c.ns, name, pt, data, subresources...), &v1alpha1.DesktopWorkspace{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopWorkspace), err
}
