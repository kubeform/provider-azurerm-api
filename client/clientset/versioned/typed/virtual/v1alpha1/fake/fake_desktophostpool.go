/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/virtual/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDesktopHostPools implements DesktopHostPoolInterface
type FakeDesktopHostPools struct {
	Fake *FakeVirtualV1alpha1
	ns   string
}

var desktophostpoolsResource = schema.GroupVersionResource{Group: "virtual.azurerm.kubeform.com", Version: "v1alpha1", Resource: "desktophostpools"}

var desktophostpoolsKind = schema.GroupVersionKind{Group: "virtual.azurerm.kubeform.com", Version: "v1alpha1", Kind: "DesktopHostPool"}

// Get takes name of the desktopHostPool, and returns the corresponding desktopHostPool object, and an error if there is any.
func (c *FakeDesktopHostPools) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DesktopHostPool, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(desktophostpoolsResource, c.ns, name), &v1alpha1.DesktopHostPool{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopHostPool), err
}

// List takes label and field selectors, and returns the list of DesktopHostPools that match those selectors.
func (c *FakeDesktopHostPools) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DesktopHostPoolList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(desktophostpoolsResource, desktophostpoolsKind, c.ns, opts), &v1alpha1.DesktopHostPoolList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DesktopHostPoolList{ListMeta: obj.(*v1alpha1.DesktopHostPoolList).ListMeta}
	for _, item := range obj.(*v1alpha1.DesktopHostPoolList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested desktopHostPools.
func (c *FakeDesktopHostPools) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(desktophostpoolsResource, c.ns, opts))

}

// Create takes the representation of a desktopHostPool and creates it.  Returns the server's representation of the desktopHostPool, and an error, if there is any.
func (c *FakeDesktopHostPools) Create(ctx context.Context, desktopHostPool *v1alpha1.DesktopHostPool, opts v1.CreateOptions) (result *v1alpha1.DesktopHostPool, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(desktophostpoolsResource, c.ns, desktopHostPool), &v1alpha1.DesktopHostPool{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopHostPool), err
}

// Update takes the representation of a desktopHostPool and updates it. Returns the server's representation of the desktopHostPool, and an error, if there is any.
func (c *FakeDesktopHostPools) Update(ctx context.Context, desktopHostPool *v1alpha1.DesktopHostPool, opts v1.UpdateOptions) (result *v1alpha1.DesktopHostPool, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(desktophostpoolsResource, c.ns, desktopHostPool), &v1alpha1.DesktopHostPool{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopHostPool), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDesktopHostPools) UpdateStatus(ctx context.Context, desktopHostPool *v1alpha1.DesktopHostPool, opts v1.UpdateOptions) (*v1alpha1.DesktopHostPool, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(desktophostpoolsResource, "status", c.ns, desktopHostPool), &v1alpha1.DesktopHostPool{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopHostPool), err
}

// Delete takes name of the desktopHostPool and deletes it. Returns an error if one occurs.
func (c *FakeDesktopHostPools) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(desktophostpoolsResource, c.ns, name), &v1alpha1.DesktopHostPool{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDesktopHostPools) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(desktophostpoolsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DesktopHostPoolList{})
	return err
}

// Patch applies the patch and returns the patched desktopHostPool.
func (c *FakeDesktopHostPools) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DesktopHostPool, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(desktophostpoolsResource, c.ns, name, pt, data, subresources...), &v1alpha1.DesktopHostPool{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DesktopHostPool), err
}
