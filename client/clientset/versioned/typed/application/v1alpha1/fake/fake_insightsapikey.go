/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/application/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeInsightsAPIKeys implements InsightsAPIKeyInterface
type FakeInsightsAPIKeys struct {
	Fake *FakeApplicationV1alpha1
	ns   string
}

var insightsapikeysResource = schema.GroupVersionResource{Group: "application.azurerm.kubeform.com", Version: "v1alpha1", Resource: "insightsapikeys"}

var insightsapikeysKind = schema.GroupVersionKind{Group: "application.azurerm.kubeform.com", Version: "v1alpha1", Kind: "InsightsAPIKey"}

// Get takes name of the insightsAPIKey, and returns the corresponding insightsAPIKey object, and an error if there is any.
func (c *FakeInsightsAPIKeys) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.InsightsAPIKey, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(insightsapikeysResource, c.ns, name), &v1alpha1.InsightsAPIKey{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.InsightsAPIKey), err
}

// List takes label and field selectors, and returns the list of InsightsAPIKeys that match those selectors.
func (c *FakeInsightsAPIKeys) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.InsightsAPIKeyList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(insightsapikeysResource, insightsapikeysKind, c.ns, opts), &v1alpha1.InsightsAPIKeyList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.InsightsAPIKeyList{ListMeta: obj.(*v1alpha1.InsightsAPIKeyList).ListMeta}
	for _, item := range obj.(*v1alpha1.InsightsAPIKeyList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested insightsAPIKeys.
func (c *FakeInsightsAPIKeys) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(insightsapikeysResource, c.ns, opts))

}

// Create takes the representation of a insightsAPIKey and creates it.  Returns the server's representation of the insightsAPIKey, and an error, if there is any.
func (c *FakeInsightsAPIKeys) Create(ctx context.Context, insightsAPIKey *v1alpha1.InsightsAPIKey, opts v1.CreateOptions) (result *v1alpha1.InsightsAPIKey, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(insightsapikeysResource, c.ns, insightsAPIKey), &v1alpha1.InsightsAPIKey{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.InsightsAPIKey), err
}

// Update takes the representation of a insightsAPIKey and updates it. Returns the server's representation of the insightsAPIKey, and an error, if there is any.
func (c *FakeInsightsAPIKeys) Update(ctx context.Context, insightsAPIKey *v1alpha1.InsightsAPIKey, opts v1.UpdateOptions) (result *v1alpha1.InsightsAPIKey, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(insightsapikeysResource, c.ns, insightsAPIKey), &v1alpha1.InsightsAPIKey{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.InsightsAPIKey), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeInsightsAPIKeys) UpdateStatus(ctx context.Context, insightsAPIKey *v1alpha1.InsightsAPIKey, opts v1.UpdateOptions) (*v1alpha1.InsightsAPIKey, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(insightsapikeysResource, "status", c.ns, insightsAPIKey), &v1alpha1.InsightsAPIKey{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.InsightsAPIKey), err
}

// Delete takes name of the insightsAPIKey and deletes it. Returns an error if one occurs.
func (c *FakeInsightsAPIKeys) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(insightsapikeysResource, c.ns, name), &v1alpha1.InsightsAPIKey{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeInsightsAPIKeys) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(insightsapikeysResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.InsightsAPIKeyList{})
	return err
}

// Patch applies the patch and returns the patched insightsAPIKey.
func (c *FakeInsightsAPIKeys) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.InsightsAPIKey, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(insightsapikeysResource, c.ns, name, pt, data, subresources...), &v1alpha1.InsightsAPIKey{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.InsightsAPIKey), err
}
