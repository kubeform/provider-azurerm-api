/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/loganalytics/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeStorageInsightses implements StorageInsightsInterface
type FakeStorageInsightses struct {
	Fake *FakeLoganalyticsV1alpha1
	ns   string
}

var storageinsightsesResource = schema.GroupVersionResource{Group: "loganalytics.azurerm.kubeform.com", Version: "v1alpha1", Resource: "storageinsightses"}

var storageinsightsesKind = schema.GroupVersionKind{Group: "loganalytics.azurerm.kubeform.com", Version: "v1alpha1", Kind: "StorageInsights"}

// Get takes name of the storageInsights, and returns the corresponding storageInsights object, and an error if there is any.
func (c *FakeStorageInsightses) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.StorageInsights, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(storageinsightsesResource, c.ns, name), &v1alpha1.StorageInsights{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StorageInsights), err
}

// List takes label and field selectors, and returns the list of StorageInsightses that match those selectors.
func (c *FakeStorageInsightses) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.StorageInsightsList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(storageinsightsesResource, storageinsightsesKind, c.ns, opts), &v1alpha1.StorageInsightsList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.StorageInsightsList{ListMeta: obj.(*v1alpha1.StorageInsightsList).ListMeta}
	for _, item := range obj.(*v1alpha1.StorageInsightsList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested storageInsightses.
func (c *FakeStorageInsightses) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(storageinsightsesResource, c.ns, opts))

}

// Create takes the representation of a storageInsights and creates it.  Returns the server's representation of the storageInsights, and an error, if there is any.
func (c *FakeStorageInsightses) Create(ctx context.Context, storageInsights *v1alpha1.StorageInsights, opts v1.CreateOptions) (result *v1alpha1.StorageInsights, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(storageinsightsesResource, c.ns, storageInsights), &v1alpha1.StorageInsights{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StorageInsights), err
}

// Update takes the representation of a storageInsights and updates it. Returns the server's representation of the storageInsights, and an error, if there is any.
func (c *FakeStorageInsightses) Update(ctx context.Context, storageInsights *v1alpha1.StorageInsights, opts v1.UpdateOptions) (result *v1alpha1.StorageInsights, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(storageinsightsesResource, c.ns, storageInsights), &v1alpha1.StorageInsights{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StorageInsights), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeStorageInsightses) UpdateStatus(ctx context.Context, storageInsights *v1alpha1.StorageInsights, opts v1.UpdateOptions) (*v1alpha1.StorageInsights, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(storageinsightsesResource, "status", c.ns, storageInsights), &v1alpha1.StorageInsights{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StorageInsights), err
}

// Delete takes name of the storageInsights and deletes it. Returns an error if one occurs.
func (c *FakeStorageInsightses) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(storageinsightsesResource, c.ns, name), &v1alpha1.StorageInsights{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeStorageInsightses) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(storageinsightsesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.StorageInsightsList{})
	return err
}

// Patch applies the patch and returns the patched storageInsights.
func (c *FakeStorageInsightses) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.StorageInsights, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(storageinsightsesResource, c.ns, name, pt, data, subresources...), &v1alpha1.StorageInsights{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StorageInsights), err
}
