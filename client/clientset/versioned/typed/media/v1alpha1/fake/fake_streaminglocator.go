/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/media/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeStreamingLocators implements StreamingLocatorInterface
type FakeStreamingLocators struct {
	Fake *FakeMediaV1alpha1
	ns   string
}

var streaminglocatorsResource = schema.GroupVersionResource{Group: "media.azurerm.kubeform.com", Version: "v1alpha1", Resource: "streaminglocators"}

var streaminglocatorsKind = schema.GroupVersionKind{Group: "media.azurerm.kubeform.com", Version: "v1alpha1", Kind: "StreamingLocator"}

// Get takes name of the streamingLocator, and returns the corresponding streamingLocator object, and an error if there is any.
func (c *FakeStreamingLocators) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.StreamingLocator, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(streaminglocatorsResource, c.ns, name), &v1alpha1.StreamingLocator{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingLocator), err
}

// List takes label and field selectors, and returns the list of StreamingLocators that match those selectors.
func (c *FakeStreamingLocators) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.StreamingLocatorList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(streaminglocatorsResource, streaminglocatorsKind, c.ns, opts), &v1alpha1.StreamingLocatorList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.StreamingLocatorList{ListMeta: obj.(*v1alpha1.StreamingLocatorList).ListMeta}
	for _, item := range obj.(*v1alpha1.StreamingLocatorList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested streamingLocators.
func (c *FakeStreamingLocators) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(streaminglocatorsResource, c.ns, opts))

}

// Create takes the representation of a streamingLocator and creates it.  Returns the server's representation of the streamingLocator, and an error, if there is any.
func (c *FakeStreamingLocators) Create(ctx context.Context, streamingLocator *v1alpha1.StreamingLocator, opts v1.CreateOptions) (result *v1alpha1.StreamingLocator, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(streaminglocatorsResource, c.ns, streamingLocator), &v1alpha1.StreamingLocator{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingLocator), err
}

// Update takes the representation of a streamingLocator and updates it. Returns the server's representation of the streamingLocator, and an error, if there is any.
func (c *FakeStreamingLocators) Update(ctx context.Context, streamingLocator *v1alpha1.StreamingLocator, opts v1.UpdateOptions) (result *v1alpha1.StreamingLocator, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(streaminglocatorsResource, c.ns, streamingLocator), &v1alpha1.StreamingLocator{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingLocator), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeStreamingLocators) UpdateStatus(ctx context.Context, streamingLocator *v1alpha1.StreamingLocator, opts v1.UpdateOptions) (*v1alpha1.StreamingLocator, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(streaminglocatorsResource, "status", c.ns, streamingLocator), &v1alpha1.StreamingLocator{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingLocator), err
}

// Delete takes name of the streamingLocator and deletes it. Returns an error if one occurs.
func (c *FakeStreamingLocators) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(streaminglocatorsResource, c.ns, name), &v1alpha1.StreamingLocator{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeStreamingLocators) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(streaminglocatorsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.StreamingLocatorList{})
	return err
}

// Patch applies the patch and returns the patched streamingLocator.
func (c *FakeStreamingLocators) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.StreamingLocator, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(streaminglocatorsResource, c.ns, name, pt, data, subresources...), &v1alpha1.StreamingLocator{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingLocator), err
}
