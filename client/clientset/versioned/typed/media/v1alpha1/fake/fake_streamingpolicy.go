/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/media/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeStreamingPolicies implements StreamingPolicyInterface
type FakeStreamingPolicies struct {
	Fake *FakeMediaV1alpha1
	ns   string
}

var streamingpoliciesResource = schema.GroupVersionResource{Group: "media.azurerm.kubeform.com", Version: "v1alpha1", Resource: "streamingpolicies"}

var streamingpoliciesKind = schema.GroupVersionKind{Group: "media.azurerm.kubeform.com", Version: "v1alpha1", Kind: "StreamingPolicy"}

// Get takes name of the streamingPolicy, and returns the corresponding streamingPolicy object, and an error if there is any.
func (c *FakeStreamingPolicies) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.StreamingPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(streamingpoliciesResource, c.ns, name), &v1alpha1.StreamingPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingPolicy), err
}

// List takes label and field selectors, and returns the list of StreamingPolicies that match those selectors.
func (c *FakeStreamingPolicies) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.StreamingPolicyList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(streamingpoliciesResource, streamingpoliciesKind, c.ns, opts), &v1alpha1.StreamingPolicyList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.StreamingPolicyList{ListMeta: obj.(*v1alpha1.StreamingPolicyList).ListMeta}
	for _, item := range obj.(*v1alpha1.StreamingPolicyList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested streamingPolicies.
func (c *FakeStreamingPolicies) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(streamingpoliciesResource, c.ns, opts))

}

// Create takes the representation of a streamingPolicy and creates it.  Returns the server's representation of the streamingPolicy, and an error, if there is any.
func (c *FakeStreamingPolicies) Create(ctx context.Context, streamingPolicy *v1alpha1.StreamingPolicy, opts v1.CreateOptions) (result *v1alpha1.StreamingPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(streamingpoliciesResource, c.ns, streamingPolicy), &v1alpha1.StreamingPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingPolicy), err
}

// Update takes the representation of a streamingPolicy and updates it. Returns the server's representation of the streamingPolicy, and an error, if there is any.
func (c *FakeStreamingPolicies) Update(ctx context.Context, streamingPolicy *v1alpha1.StreamingPolicy, opts v1.UpdateOptions) (result *v1alpha1.StreamingPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(streamingpoliciesResource, c.ns, streamingPolicy), &v1alpha1.StreamingPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingPolicy), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeStreamingPolicies) UpdateStatus(ctx context.Context, streamingPolicy *v1alpha1.StreamingPolicy, opts v1.UpdateOptions) (*v1alpha1.StreamingPolicy, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(streamingpoliciesResource, "status", c.ns, streamingPolicy), &v1alpha1.StreamingPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingPolicy), err
}

// Delete takes name of the streamingPolicy and deletes it. Returns an error if one occurs.
func (c *FakeStreamingPolicies) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(streamingpoliciesResource, c.ns, name), &v1alpha1.StreamingPolicy{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeStreamingPolicies) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(streamingpoliciesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.StreamingPolicyList{})
	return err
}

// Patch applies the patch and returns the patched streamingPolicy.
func (c *FakeStreamingPolicies) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.StreamingPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(streamingpoliciesResource, c.ns, name, pt, data, subresources...), &v1alpha1.StreamingPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.StreamingPolicy), err
}
