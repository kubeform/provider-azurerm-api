/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/media/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeLiveEventOutputs implements LiveEventOutputInterface
type FakeLiveEventOutputs struct {
	Fake *FakeMediaV1alpha1
	ns   string
}

var liveeventoutputsResource = schema.GroupVersionResource{Group: "media.azurerm.kubeform.com", Version: "v1alpha1", Resource: "liveeventoutputs"}

var liveeventoutputsKind = schema.GroupVersionKind{Group: "media.azurerm.kubeform.com", Version: "v1alpha1", Kind: "LiveEventOutput"}

// Get takes name of the liveEventOutput, and returns the corresponding liveEventOutput object, and an error if there is any.
func (c *FakeLiveEventOutputs) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.LiveEventOutput, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(liveeventoutputsResource, c.ns, name), &v1alpha1.LiveEventOutput{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.LiveEventOutput), err
}

// List takes label and field selectors, and returns the list of LiveEventOutputs that match those selectors.
func (c *FakeLiveEventOutputs) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.LiveEventOutputList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(liveeventoutputsResource, liveeventoutputsKind, c.ns, opts), &v1alpha1.LiveEventOutputList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.LiveEventOutputList{ListMeta: obj.(*v1alpha1.LiveEventOutputList).ListMeta}
	for _, item := range obj.(*v1alpha1.LiveEventOutputList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested liveEventOutputs.
func (c *FakeLiveEventOutputs) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(liveeventoutputsResource, c.ns, opts))

}

// Create takes the representation of a liveEventOutput and creates it.  Returns the server's representation of the liveEventOutput, and an error, if there is any.
func (c *FakeLiveEventOutputs) Create(ctx context.Context, liveEventOutput *v1alpha1.LiveEventOutput, opts v1.CreateOptions) (result *v1alpha1.LiveEventOutput, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(liveeventoutputsResource, c.ns, liveEventOutput), &v1alpha1.LiveEventOutput{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.LiveEventOutput), err
}

// Update takes the representation of a liveEventOutput and updates it. Returns the server's representation of the liveEventOutput, and an error, if there is any.
func (c *FakeLiveEventOutputs) Update(ctx context.Context, liveEventOutput *v1alpha1.LiveEventOutput, opts v1.UpdateOptions) (result *v1alpha1.LiveEventOutput, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(liveeventoutputsResource, c.ns, liveEventOutput), &v1alpha1.LiveEventOutput{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.LiveEventOutput), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeLiveEventOutputs) UpdateStatus(ctx context.Context, liveEventOutput *v1alpha1.LiveEventOutput, opts v1.UpdateOptions) (*v1alpha1.LiveEventOutput, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(liveeventoutputsResource, "status", c.ns, liveEventOutput), &v1alpha1.LiveEventOutput{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.LiveEventOutput), err
}

// Delete takes name of the liveEventOutput and deletes it. Returns an error if one occurs.
func (c *FakeLiveEventOutputs) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(liveeventoutputsResource, c.ns, name), &v1alpha1.LiveEventOutput{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeLiveEventOutputs) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(liveeventoutputsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.LiveEventOutputList{})
	return err
}

// Patch applies the patch and returns the patched liveEventOutput.
func (c *FakeLiveEventOutputs) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.LiveEventOutput, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(liveeventoutputsResource, c.ns, name, pt, data, subresources...), &v1alpha1.LiveEventOutput{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.LiveEventOutput), err
}
