/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/storage/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeEncryptionScopes implements EncryptionScopeInterface
type FakeEncryptionScopes struct {
	Fake *FakeStorageV1alpha1
	ns   string
}

var encryptionscopesResource = schema.GroupVersionResource{Group: "storage.azurerm.kubeform.com", Version: "v1alpha1", Resource: "encryptionscopes"}

var encryptionscopesKind = schema.GroupVersionKind{Group: "storage.azurerm.kubeform.com", Version: "v1alpha1", Kind: "EncryptionScope"}

// Get takes name of the encryptionScope, and returns the corresponding encryptionScope object, and an error if there is any.
func (c *FakeEncryptionScopes) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.EncryptionScope, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(encryptionscopesResource, c.ns, name), &v1alpha1.EncryptionScope{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionScope), err
}

// List takes label and field selectors, and returns the list of EncryptionScopes that match those selectors.
func (c *FakeEncryptionScopes) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.EncryptionScopeList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(encryptionscopesResource, encryptionscopesKind, c.ns, opts), &v1alpha1.EncryptionScopeList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.EncryptionScopeList{ListMeta: obj.(*v1alpha1.EncryptionScopeList).ListMeta}
	for _, item := range obj.(*v1alpha1.EncryptionScopeList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested encryptionScopes.
func (c *FakeEncryptionScopes) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(encryptionscopesResource, c.ns, opts))

}

// Create takes the representation of a encryptionScope and creates it.  Returns the server's representation of the encryptionScope, and an error, if there is any.
func (c *FakeEncryptionScopes) Create(ctx context.Context, encryptionScope *v1alpha1.EncryptionScope, opts v1.CreateOptions) (result *v1alpha1.EncryptionScope, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(encryptionscopesResource, c.ns, encryptionScope), &v1alpha1.EncryptionScope{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionScope), err
}

// Update takes the representation of a encryptionScope and updates it. Returns the server's representation of the encryptionScope, and an error, if there is any.
func (c *FakeEncryptionScopes) Update(ctx context.Context, encryptionScope *v1alpha1.EncryptionScope, opts v1.UpdateOptions) (result *v1alpha1.EncryptionScope, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(encryptionscopesResource, c.ns, encryptionScope), &v1alpha1.EncryptionScope{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionScope), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeEncryptionScopes) UpdateStatus(ctx context.Context, encryptionScope *v1alpha1.EncryptionScope, opts v1.UpdateOptions) (*v1alpha1.EncryptionScope, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(encryptionscopesResource, "status", c.ns, encryptionScope), &v1alpha1.EncryptionScope{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionScope), err
}

// Delete takes name of the encryptionScope and deletes it. Returns an error if one occurs.
func (c *FakeEncryptionScopes) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(encryptionscopesResource, c.ns, name), &v1alpha1.EncryptionScope{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeEncryptionScopes) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(encryptionscopesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.EncryptionScopeList{})
	return err
}

// Patch applies the patch and returns the patched encryptionScope.
func (c *FakeEncryptionScopes) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.EncryptionScope, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(encryptionscopesResource, c.ns, name, pt, data, subresources...), &v1alpha1.EncryptionScope{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionScope), err
}
