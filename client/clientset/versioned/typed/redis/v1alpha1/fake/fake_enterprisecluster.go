/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-azurerm-api/apis/redis/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeEnterpriseClusters implements EnterpriseClusterInterface
type FakeEnterpriseClusters struct {
	Fake *FakeRedisV1alpha1
	ns   string
}

var enterpriseclustersResource = schema.GroupVersionResource{Group: "redis.azurerm.kubeform.com", Version: "v1alpha1", Resource: "enterpriseclusters"}

var enterpriseclustersKind = schema.GroupVersionKind{Group: "redis.azurerm.kubeform.com", Version: "v1alpha1", Kind: "EnterpriseCluster"}

// Get takes name of the enterpriseCluster, and returns the corresponding enterpriseCluster object, and an error if there is any.
func (c *FakeEnterpriseClusters) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.EnterpriseCluster, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(enterpriseclustersResource, c.ns, name), &v1alpha1.EnterpriseCluster{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EnterpriseCluster), err
}

// List takes label and field selectors, and returns the list of EnterpriseClusters that match those selectors.
func (c *FakeEnterpriseClusters) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.EnterpriseClusterList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(enterpriseclustersResource, enterpriseclustersKind, c.ns, opts), &v1alpha1.EnterpriseClusterList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.EnterpriseClusterList{ListMeta: obj.(*v1alpha1.EnterpriseClusterList).ListMeta}
	for _, item := range obj.(*v1alpha1.EnterpriseClusterList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested enterpriseClusters.
func (c *FakeEnterpriseClusters) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(enterpriseclustersResource, c.ns, opts))

}

// Create takes the representation of a enterpriseCluster and creates it.  Returns the server's representation of the enterpriseCluster, and an error, if there is any.
func (c *FakeEnterpriseClusters) Create(ctx context.Context, enterpriseCluster *v1alpha1.EnterpriseCluster, opts v1.CreateOptions) (result *v1alpha1.EnterpriseCluster, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(enterpriseclustersResource, c.ns, enterpriseCluster), &v1alpha1.EnterpriseCluster{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EnterpriseCluster), err
}

// Update takes the representation of a enterpriseCluster and updates it. Returns the server's representation of the enterpriseCluster, and an error, if there is any.
func (c *FakeEnterpriseClusters) Update(ctx context.Context, enterpriseCluster *v1alpha1.EnterpriseCluster, opts v1.UpdateOptions) (result *v1alpha1.EnterpriseCluster, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(enterpriseclustersResource, c.ns, enterpriseCluster), &v1alpha1.EnterpriseCluster{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EnterpriseCluster), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeEnterpriseClusters) UpdateStatus(ctx context.Context, enterpriseCluster *v1alpha1.EnterpriseCluster, opts v1.UpdateOptions) (*v1alpha1.EnterpriseCluster, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(enterpriseclustersResource, "status", c.ns, enterpriseCluster), &v1alpha1.EnterpriseCluster{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EnterpriseCluster), err
}

// Delete takes name of the enterpriseCluster and deletes it. Returns an error if one occurs.
func (c *FakeEnterpriseClusters) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(enterpriseclustersResource, c.ns, name), &v1alpha1.EnterpriseCluster{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeEnterpriseClusters) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(enterpriseclustersResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.EnterpriseClusterList{})
	return err
}

// Patch applies the patch and returns the patched enterpriseCluster.
func (c *FakeEnterpriseClusters) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.EnterpriseCluster, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(enterpriseclustersResource, c.ns, name, pt, data, subresources...), &v1alpha1.EnterpriseCluster{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EnterpriseCluster), err
}
