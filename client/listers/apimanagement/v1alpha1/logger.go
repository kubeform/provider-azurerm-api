...
// List lists all Loggers in the indexer.
func (s *loggerLister) List(selector labels.Selector) (ret []*v1alpha1.Logger, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Logger))
	})
	return ret, err
}
...
// List lists all Loggers in the indexer for a given namespace.
func (s loggerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Logger, err error) {
...
// Get retrieves the Logger from the indexer for a given namespace and name.
func (s loggerNamespaceLister) Get(name string) (*v1alpha1.Logger, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("logger"), name)
	}
	return obj.(*v1alpha1.Logger), nil
}
