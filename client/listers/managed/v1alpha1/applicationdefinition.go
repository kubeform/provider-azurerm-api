...
type ApplicationDefinitionLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ApplicationDefinition, err error)
...
// List lists all ApplicationDefinitions in the indexer.
func (s *applicationDefinitionLister) List(selector labels.Selector) (ret []*v1alpha1.ApplicationDefinition, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ApplicationDefinition))
	})
	return ret, err
}
...
type ApplicationDefinitionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ApplicationDefinition, error)
...
// List lists all ApplicationDefinitions in the indexer for a given namespace.
func (s applicationDefinitionNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ApplicationDefinition, err error) {
...
// Get retrieves the ApplicationDefinition from the indexer for a given namespace and name.
func (s applicationDefinitionNamespaceLister) Get(name string) (*v1alpha1.ApplicationDefinition, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("applicationdefinition"), name)
	}
	return obj.(*v1alpha1.ApplicationDefinition), nil
}
